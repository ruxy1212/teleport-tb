...
export default function Home() {
    ...
    useEffect(() => {
		const getProducts = async () => {
			try {
				const apiKey = process.env.TIMBU_API_KEY;
                const appId = process.env.TIMBU_APP_ID;
                const orgId = process.env.TIMBU_ORG_ID;
                const flashId = process.env.TIMBU_FLASH_ID;
                const mainId = process.env.TIMBU_MAIN_ID;
				const main_products = await fetch(`/products?organization_id=${orgId}&Appid=${appId}&Apikey=${apiKey}&reverse_sort=false&&category_id=${mainId}&size=12&page=${currentPage}`);
                const flash_products = await fetch(`/products?organization_id=${orgId}&Appid=${appId}&Apikey=${apiKey}&reverse_sort=false&&category_id=${flashId}&size=12&page=1`);
				if (main_products.ok && flash_products.ok) {
                    let main = main_products.json(), flash = flash_products.json();
                    setTotalPages(Math.ceil(main.total/main.size));
                    setMainProducts(main.items);
                    setFlashProducts(flash.items);
					setError(false);
				}
			}catch(error){
