...
type FlashProductProps = {
    ...
    addToCart: (product: FlashProductProps["product"]) => void
}
//{product, addToCart}props.
export default function FlashProduct(props: FlashProductProps){
    ...
    return(
        <div className="flex flex-col justify-between">
            <div className="flex flex-col">
                <div onClick={()=>setShowProduct(true)} className="cursor-pointer flex flex-col px-8 py-4 w-full rounded-2xl shadow-lg border border-pd-white backdrop-blur-sm aspect-square leading-[120%]">
                    <div className="relative flex flex-col justify-center mt-2">
                        {loaded && showProduct && <div className="flex justify-end">
                            <span className="flex justify-center items-center h-[60px] w-[135px] bg-[url('/images/callout.svg')] pd-p-18 font-bold relative bottom-[-23px] right-[-23px] text-pd-white z-10">{Math.ceil(props.product.discount/props.product.price*100)}% Off</span>
                        </div>}
                        <img onLoad={handleImageLoad} src={baseURL+props.product.photo} className={` hover:scale-110 transition-all duration-500 object-cover h-[250px] w-[250px] ${loaded?'block':'hidden'}`} />
                        ...
                        <div className={`flex gap-3 self-center mt-4 ${loaded?'':'bg-pd-gray animate-pulse'}`}>
                            <h3 className={`text-[26px] font-montserrat font-bold text-pd-black ${loaded?'':'opacity-0'}`}>${props.product.price-props.product.discount}</h3>
                            <div className={`my-auto text-sm font-medium font-montserrat text-pd-red line-through ${loaded?'':'opacity-0'}`}>${props.product.price}</div>
                        </div>
                    </div>
                </div>
                <div className={`flex gap-5 mt-10 text-2xl font-extrabold text-pd-black ${loaded?'':'h-5 bg-pd-gray animate-pulse'}`}>
                    <h3 className={`leading-8 pd-h3 ${loaded?'':'hidden'}`}>{props.product.title}</h3>
                </div>
                {loaded && 
                    <p className="mt-6 leading-6 text-pd-black pd-p">{props.product.desc}</p>
                }
                {loaded && 
                    <div className="flex gap-2.5 self-start pr-5 mt-4">
                        <div className="flex gap-0.5 my-auto">
                            <Rating amount={props.product.rating}/>
                        </div>
            ...
            <div>
                <button disabled={!loaded} className={`${loaded?'':'bg-pd-gray hover:bg-pd-gray transition-none text-pd-gray active:shadow-none hover:text-pd-gray'} white-to-black-border flex gap-2.5 justify-center px-9 py-4 mt-5 text-base font-medium leading-5 text-black border border-black border-solid rounded-[52px] with-shadow`} onClick={() => props.addToCart(props.product)}>
                    <Cart />
                    <p>Add to Cart</p>
                </button> 
            </div>
